<template>
  <table v-if="this.totales.activos" id="customers">
    ...
    <tr
      ...
      :key="index"
      @click="this.$router.push(`/activos/${activo._id}`)"
    >
      ...
      <td>{{ activo.suplidores }}</td>
      <td class="der">{{ formatNumber(activo.costo, true) }}</td>
    </tr>
    <tr>
...
import { updateOne } from "@/services/almaycru/Activo";
...
export default {
  ...
  methods: {
    ...
    valorTotal() {
      ...
      this.totales.cant = this.activos.reduce(
        (accum: any, item: any) => accum + item.cant,
        0
      );
      this.totales.valor = this.activos.reduce(
        (accum: any, item: any) => accum + item.costo,
        0
      );
    },
