<template>
  <table v-if="this.totales.jornadas" id="customers">
    <tr>
      <th>Fecha</th>
      <th>Empleados</th>
    </tr>
  ...
  <h6 class="noRegist" v-if="!this.totales.jornadas">
...
import { updateOne } from "@/services/almaycru/Jornada";
...
export default {
  ...
  methods: {
    ...
    async marcarListo(jornada: Jornada) {
      alert("Jornada Lista.");
      try {
        const res = await updateOne(jornada);
      } catch (error) {
        // console.error(error);
      }
    },
    ...
    valorTotal() {
      this.totales.jornadas = this.jornadas.length;
      this.totales.valor = this.jornadas.reduce(
