<template>
  ...
  <body v-show="!cargando">
    <div class="grid-layout">
      ...
      <div class="caja c1">
        <h4 class="bold">{{ this.pago.medico }}</h4>
        <h5>{{ this.pago.direccion }}</h5>
        <!-- <h5>
          <span class="bold">RNC / Cédula: </span>
          <span>{{ this.pago.rncMedico }}</span>
        </h5> -->
        <h5>
      ...
      <div class="caja c2">
        <h3 class="bold">Pago<br /></h3>
        <h5>
          <span class="bold">No: </span>
          <span>{{ formatSecuence(pago.no) }}</span>
        </h5>
        <!-- <h5>
        ...
        <h5>
          ...
          <span>{{ formatDate(this.pago.fechaProceso) }}</span>
        </h5>
        <!-- <h5>
          <span class="bold">Vencimiento: </span>
          <span>{{ formatDate(this.pago.vencimiento) }}</span>
        </h5> -->
      </div>
      ...
      <div class="caja c3">
        <!-- <h5>
          ...
          <span>{{ this.pago.rncCliente }}</span>
        </h5> -->
        <h5>
          <span class="bold">Cliente: </span>
          <span>{{ this.pago.descCliente }}</span>
        </h5>
      </div>
      <div class="caja c4">
        <table class="table table-hover">
          ...
          <tr>
            <td class="t izq">{{ this.pago.descServicios }}</td>
            <td class="t der">
              {{ formatNumber(this.pago.prepago) }}
            </td>
          </tr>
        </table>
      </div>
      ...
      <div class="caja c6">
        <h6>{{ formatNumber(this.pago.prepago) }}</h6>
        <h6>{{ formatNumber(this.pago.descuento) }}</h6>
        <h6>{{ formatNumber(this.pago.descuento) }}</h6>
        <h6 class="bold">
          {{ formatNumber(this.pago.prepago) }}
        </h6>
...
export default defineComponent({
  ...
  methods: {
    ...
    async handleDelete() {
      try {
        if (typeof this.$route.params.id === "string") {
          deletePago(this.$route.params.id);
          this.$router.push("/pagos");
        }
      } catch (error) {
        //console.error(error);
  ...
  mounted() {
    if (typeof this.$route.params.id === "string") {
      this.loadPago(this.$route.params.id);
    }
  },
