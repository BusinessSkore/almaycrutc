    ...
    <Transition>
      <div v-show="!cargando" class="general">
        ...
        > -->
            ...
            <ListadoComponent :pagos="pagos" v-show="!cargando" />
          </div>
        </div>
      </div>
    </Transition>
...
<script lang="ts">
import Filters from "@/components/Pago/FiltersComponent.vue";
import ListadoComponent from "@/components/Pago/ListComponent.vue";
import Navbar from "@/components/Navbar.vue";
import { Pago } from "@/interfaces/Pago";
import { getPagos } from "@/services/almaycru/Pago";
...
export default {
  ...
  methods: {
    ...
    async filterPagos(catName: string) {
      try {
      ...
      if (catName !== "Todos") {
        this.pagos = this.pagos.filter((pago: Pago) => {
          return pago.empleado === catName;
        });
      }
    },
    ...
    async search(term: string) {
      ...
      if (term !== "Todos") {
        this.pagos = this.pagos.filter((pago: Pago) => {
          return pago.empleado.toLowerCase().includes(term.toLowerCase());
        });
      }
      this.toggleLoading();
    },
...
.grid {
  ...
  grid-template-areas:
    ...
    "pagos pagos pagos pagos pagos pagos pagos";
}
