<template>
  <table v-if="this.totales.pagos" id="customers">
    ...
    <tr
      ...
      :key="index"
      @click="this.$router.push(`/pagos/${pago._id}`)"
    >
  ...
  <h6 class="noRegist" v-if="!this.totales.pagos">
...
import { updateOne } from "@/services/almaycru/Pago";
...
export default {
  ...
  methods: {
    ...
    async marcarListo(pago: Pago) {
      alert("Pago Lista.");
      try {
        const res = await updateOne(pago);
      } catch (error) {
        // console.error(error);
      }
    },
