<template>
  <table v-if="this.totales.monos" id="customers">
    <tr>
      <th>Fecha</th>
      <th>Empleado</th>
      <th>Vitola</th>
    ...
    <tr
      v-for="(mono, index) in monos"
      :key="index"
      @click="this.$router.push(`/monos/${mono._id}`)"
    >
      <td>{{ formatDate(mono.fecha) }}</td>
      <td>{{ mono.empleadoEmpunero }}</td>
      <td>{{ mono.vitola }}</td>
      <!-- <td class="der">{{ formatNumber(mono.count, false) }}</td> -->
      <td class="der">{{ formatNumber(mono.cantidad, false) }}</td>
    </tr>
    <tr>
      <td>Total: {{ formatNumber(this.totales.monos) }}</td>
      <td></td>
      <td></td>
  ...
  <h6 class="noRegist" v-if="!this.totales.monos">
...
import { updateOne } from "@/services/almaycru/Mono";
...
export default {
  ...
  methods: {
    ...
    async marcarListo(mono: Mono) {
      alert("Mono Lista.");
      try {
        const res = await updateOne(mono);
      } catch (error) {
        // console.error(error);
      }
    },
    ...
    valorTotal() {
      this.totales.monos = this.monos.length;
      this.totales.valor = this.monos.reduce(
        (accum: any, item: any) => accum + item.monos,
        0
      );
      this.totales.cant = this.monos.reduce(
        (accum: any, item: any) => accum + item.count,
        0
      );
      this.totales.cantidad = this.monos.reduce(
