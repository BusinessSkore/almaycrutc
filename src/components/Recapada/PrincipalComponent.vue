      ...
      <div v-if="showModal" class="modal">
        <div class="contenedor">
          ...
          <div class="contenido">
            ...
            <div class="contenido">
              <Filters
                :filterRecapadas="filterRecapadas"
                :search="search"
                :filteredRecapadas="filteredRecapadas"
              />
            </div>
          </div>
        </div>
      ...
      <div class="grid">
        ...
        > -->
          <h4>
            ...
            <i
              style="cursor:pointer"
              @click="this.$router.push(`/recapadas/new`)"
              class="fas fa-plus"
              _mstvisible="2"
            ></i>
          </h4>
...
export default {
  ...
  methods: {
    ...
    async filterRecapadas(catName: string) {
      ...
      if (catName !== "Todos") {
        this.recapadas = this.recapadas.filter((recapada: Recapada) => {
          return recapada.liga === catName;
        });
      }
    },
    ...
    async search(term: string) {
      ...
      if (term !== "Todos") {
        this.recapadas = this.recapadas.filter((recapada: Recapada) => {
          return recapada.liga.toLowerCase().includes(term.toLowerCase());
        });
      }
      this.toggleLoading();
    },
...
.grid {
  ...
  grid-template-areas:
    ...
    "recapadas recapadas recapadas recapadas recapadas recapadas recapadas";
}
