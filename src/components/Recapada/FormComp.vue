        ...
        <form>
          <fieldset>
            ...
            <div class="form-group">
              <div class="grid">
                ...
                <div>
                  ...
                  ><select
                    ...
                    id="vitola"
                    type="String"
                    v-model="recapada.vitola"
                    class="form-select"
                    @change="selectVitola(recapada.vitola)"
                  >
                    <option
              ...
              <div class="grid">
                <!-- <div>
                  ...
                  ><select
                    id="empleadosMez"
                    v-model="recapada.empleadoMezclador"
                    class="form-select"
                  >
                    <option
                ...
                <!-- <div>
                  ...
                  ><select
                    :disabled="this.modoForm == 'show'"
                    id="empleadosEmp"
                    v-model="recapada.empleadoEmpunero"
                    class="form-select"
                  >
                    <option
                ...
                <div>
                  ...
                  ><select
                    :disabled="this.modoForm == 'show'"
                    id="empleadosPeg"
                    v-model="recapada.empleadoPegador"
                    class="form-select"
                  >
                    <option
            ...
            <button
              ...
              @click.prevent="saveRecapada()"
              :disabled="
                !recapada.fecha ||
                  !recapada.empleadoPegador ||
                  !recapada.vitola ||
                  !recapada.cantidad
              "
            >
            ...
            <!-- <button
              ...
              @click="this.$router.push(`/recapadas2/${recapada._id}`)"
            >
...
import {
  ...
  getRecapada,
  updateRecapada,
} from "@/services/almaycru/Recapada";
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      campoFocus: "liga",
      mensageError: "Error",
      mensageExito: "Recapada Registrada Exitosamente",
      mensageConfirm: "¿Está Seguro que desea Eliminar Esta Recapada?",
      encabezado: "",
      modoForm: "",
  ...
  async mounted() {
    if (this.$route.fullPath == "/recapadas/new") {
      this.modoForm = "add";
      this.encabezado = "Nueva Recapada";
    } else {
      this.modoForm = "show";
      this.encabezado = "Detalles de Recapada";
    }
    ...
    if (this.modoForm == "add") {
      ...
      if (typeof this.$route.params.id === "string") {
        await this.loadRecapada(this.$route.params.id);
      }
      this.fixTime();
    }
  ...
  methods: {
    calcValor() {
      this.cxp.valor =
        (this.vitolaSelected[0].pago / 50) * this.recapada.cantidad;
    },
    ...
    fixTime() {
      this.recapada.fecha = this.formatDateToFix(this.recapada.fecha, true);
    ...
    calcRecapada() {
      (this.recapada.retension = this.recapada.bruto * 0.1),
        (this.recapada.neto = this.recapada.bruto * 0.9);
    },
    ...
    async loadOneRecapada() {
      try {
        const res = await getOneRecapada();
        this.oneRecapada = res.data;
        this.one = this.oneRecapada[0];
        ...
        this.recapada.no = this.nextNo;
        this.recapada.principal = this.nextNo;
        this.recapada.principal = this.nextNo;
      } catch (error) {
        // console.error(error);
      }
    ...
    async saveRecapadaa() {
      await this.loadOneRecapada();
      try {
        const res = await createRecapadaa(this.servicio);
        // // console.log(res);
      } catch (error) {
        // // console.error(error);
      }
    ...
    async saveRecapada() {
      ...
      try {
        ...
        // await this.saveCxp();
        this.cxp.empleado = this.recapada.empleadoPegador;
        await this.saveCxp();
        const res = await createRecapada(this.recapada).then(
          (res) => {
            this.error = this.respuesta = res.data.title;
            // this.$router.push("/");
            this.res = res;
    ...
    async deleteAllRecapadas() {
      try {
        const res = await eliminateRecapadas(this.recapada);
        // // console.log(res);
      } catch (error) {
        // // console.error(error);
      }
