<template>
  ...
  <h6 class="noRegist" v-if="!this.totales.nominas">
...
import { updateOne } from "@/services/almaycru/Nomina";
...
export default {
  ...
  methods: {
    ...
    async marcarListo(nomina: Nomina) {
      alert("Nomina Lista.");
      try {
        const res = await updateOne(nomina);
      } catch (error) {
        // console.error(error);
      }
    },
    ...
    valorTotal() {
      this.totales.nominas = this.nominas.length;
      this.totales.valor = this.nominas.reduce(
