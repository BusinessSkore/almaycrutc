<template>
  ...
  <body v-show="!cargando">
    <div class="grid-layout">
      ...
      <div class="caja c1">
        <h4 class="bold">{{ this.nomina.medico }}</h4>
        <h5>{{ this.nomina.direccion }}</h5>
        <!-- <h5>
          <span class="bold">RNC / Cédula: </span>
          <span>{{ this.nomina.rncMedico }}</span>
        </h5> -->
        <h5>
      ...
      <div class="caja c2">
        <h3 class="bold">Nomina<br /></h3>
        <h5>
          <span class="bold">No: </span>
          <span>{{ formatSecuence(nomina.no) }}</span>
        </h5>
        <!-- <h5>
        ...
        <h5>
          ...
          <span>{{ formatDate(this.nomina.fechaProceso) }}</span>
        </h5>
        <!-- <h5>
          <span class="bold">Vencimiento: </span>
          <span>{{ formatDate(this.nomina.vencimiento) }}</span>
        </h5> -->
      </div>
      ...
      <div class="caja c3">
        <!-- <h5>
          ...
          <span>{{ this.nomina.rncCliente }}</span>
        </h5> -->
        <h5>
          <span class="bold">Cliente: </span>
          <span>{{ this.nomina.descCliente }}</span>
        </h5>
      </div>
      <div class="caja c4">
        <table class="table table-hover">
          ...
          <tr>
            <td class="t izq">{{ this.nomina.descServicios }}</td>
            <td class="t der">
              {{ formatNumber(this.nomina.prenomina) }}
            </td>
          </tr>
        </table>
      </div>
      ...
      <div class="caja c6">
        <h6>{{ formatNumber(this.nomina.prenomina) }}</h6>
        <h6>{{ formatNumber(this.nomina.descuento) }}</h6>
        <h6>{{ formatNumber(this.nomina.descuento) }}</h6>
        <h6 class="bold">
          {{ formatNumber(this.nomina.prenomina) }}
        </h6>
...
export default defineComponent({
  ...
  methods: {
    ...
    async loadNomina(id: string) {
      ...
      if (this.$store.state.ids.length == 0) {
        ...
        this.$router.push(`/nominaslot/new`);
      }
    ...
    async handleDelete() {
      try {
        if (typeof this.$route.params.id === "string") {
          deleteNomina(this.$route.params.id);
          this.$router.push("/nominas");
        }
      } catch (error) {
        //console.error(error);
  ...
  mounted() {
    if (typeof this.$route.params.id === "string") {
      this.loadNomina(this.$route.params.id);
    }
  },
