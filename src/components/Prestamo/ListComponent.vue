<template>
  <table v-if="this.totales.prestamos" id="customers">
    <tr>
      <th>Fecha</th>
      <th>Empleado</th>
      <th>Monto</th>
    ...
    <tr
      ...
      :key="index"
      @click="this.$router.push(`/prestamos/${prestamo._id}`)"
    >
      <td>{{ formatDate(prestamo.fecha) }}</td>
      <td>{{ prestamo.empleado }}</td>
      <td class="der">{{ formatNumber(prestamo.monto, true) }}</td>
      <td class="der">{{ formatNumber(prestamo.cuotas, false) }}</td>
      <td class="der">{{ formatNumber(prestamo.montoCuotas, true) }}</td>
    </tr>
    <tr>
      <td>Total: {{ formatNumber(this.totales.prestamos) }}</td>
      <td></td>
      <td></td>
      <td></td>
      <td></td>
  ...
  <h6 class="noRegist" v-if="!this.totales.prestamos">
...
import { updateOne } from "@/services/almaycru/Prestamo";
...
export default {
  ...
  methods: {
    ...
    async marcarListo(prestamo: Prestamo) {
      alert("Prestamo Lista.");
      try {
        const res = await updateOne(prestamo);
      } catch (error) {
        // console.error(error);
      }
    },
    ...
    valorTotal() {
      this.totales.prestamos = this.prestamos.length;
      this.totales.valor = this.prestamos.reduce(
